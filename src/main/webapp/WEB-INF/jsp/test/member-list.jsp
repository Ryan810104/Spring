...
<!-- search -->
	<nav class="navbar navbar-light bg-light">
		<form class="form-inline" action="/admin/test/query">
			...
			<button class="btn btn-outline-success my-2 my-sm-0" type="button" onclick="send()">Search</button>
		</form>
	</nav>
<!-- search end-->
	...
	<c:if test="${not empty findById}">
		<table>
			<c:url value="/admin/test/index" var="path">
...
// 			 "/admin/test/query?id1="+getUrlParameter('id1')+"&id2="+getUrlParameter('id2'),
					url : "/admin/test/query?search="+$("#search").val(),
					type : "GET",
					success : function(data){
						var str = "";
						$("#showdata")
								.append(
										"<thead><tr><th>id</th><th>password</th><th>email</th><th>phone</th><th>datetime</th></thead>");
						str += "<tbody>";
		...
		function send(){
			$(location).attr('href', '/admin/test/query?search=' + $("#search").val());
		}
		</script>
