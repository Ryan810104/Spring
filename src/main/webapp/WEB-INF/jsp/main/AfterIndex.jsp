...
<body>
	...
	<article class="content moe">
		<div id="content">
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="my-3 p-3 bg-white rounded shadow-sm">
							...
							<div class="media text-muted pt-3">
								<p
									class="media-body pb-3 mb-0 small lh-125 border-bottom border-gray">
									<strong class="d-block text-success">活動</strong> 端午活動上線啦！獎品總額高達
									<strong>1,000,000 元</strong>，詳情請入內參閱。
								</p>
							</div>
							<div class="media text-muted pt-3">
								<p
					...
					<div class="col-sm-6 my-3">
						<a class="weatherwidget-io"
							href="https://forecast7.com/zh-tw/25d03121d57/taipei/"
							data-label_1="TAIPEI" data-label_2="WEATHER"
							data-font="Noto Sans TC" data-icons="Climacons Animated"
							data-days="3" data-theme="marine"
							style="display: block; position: relative; height: 123px; padding: 0px; overflow: hidden; text-align: left; text-indent: -299rem;">TAIPEI
							WEATHER<iframe id="weatherwidget-io-0"
								class="weatherwidget-io-frame" scrolling="no" frameborder="0"
								width="100%" src="https://weatherwidget.io/w/"
								style="display: block; position: absolute; top: 0px; height: 123px;"></iframe>
						</a>
						<div class="col-sm-6 my-3" style="margin: auto;">

							<div
								style="background-color: #dce54f; text-align: center; border-radius: 3rem;">
								<h4 class="m-2" style="padding-top: 2rem; color: black;">百
									家 樂 超 級 彩 金</h4>
								<div class="odometer" style="padding: 2rem"></div>
								<br>
								<button type="button" class="btn btn-outline-success m-4">中獎名單</button>
							</div>
								...
								<div class="pull-left meta" style="width: 60%">
									<div id="putdisplaynone">
										...
										<input type="text" id="CustomermessageboardMemberNum"
											name="CustomermessageboardMemberNum"
											placeholder="此欄為隱藏抓取num格" style="display: none"
											value="${sessionScope.member.memberNum}" /> <input
											type="text" id="customermessageboardArticleFloor"
											name="customermessageboardArticleFloor" style="display: none">
										<input type="text" id="articlefloor" style="display: none"
											value="1">
										<!--隱藏搜尋樓數位置 -->
			...
			<input type="text" id="customermessageboardResponseFloor"
				style="display: none">
			<!-- 			<br> -->
			<!-- 			<div class="container"> -->
			...
			<!-- 			</div> -->
			<br>
			<div style="font-family: 'ZCOOL KuaiLe'; color: #000088"
				align="center">
				依&nbsp;&nbsp;&nbsp;&nbsp; <input type="radio" name="gender"
					value="name" id="radioname" checked="true"><label
					for="radioname"
					style="font-family: 'Permanent Marker'; color: black">&nbsp;UserID</label>
				<input type="radio" name="gender" value="title" id="radiotitle"><label
					for="radiotitle"
					style="font-family: 'Permanent Marker'; color: black">&nbsp;Title</label>
				<input type="radio" name="gender" value="content" id="radiocontent"><label
					for="radiocontent"
					style="font-family: 'Permanent Marker'; color: black">&nbsp;Content</label>
				&nbsp;&nbsp;找

				<div class="container1">
					<div class="field-input">
						<form action="/main/query" name="search">
							<input type="text" id="searchbyuserid"
								name="customermessageboardMemberid" placeholder="UserID">
							<input type="text" id="searchbytitle"
								name="customermessageboardTitle" placeholder="含有......字元">
							<input type="text" id="searchbycontent"
								name="customermessageboardMessage" placeholder="含有......字元"><span
			...
			<div class="post-footer" id="more" align="center">
				<button>
					<i class="far fa-caret-square-down"></i>&nbsp;&nbsp;查看更多&nbsp;&nbsp;<i
									...
									success : function(data) {
										...
										if (data == "") {
											...
											for (var i = 0; i < data.length; i++) {
												...
												if (flag == 1) {
													txt += "<span><i class=\"fas fa-heart\" id=\"like"
															+ parseInt(i
																	+ floor
																	* times)
															+ "\" style=\"font-size: 30px ; color: red\"><span style=\"font-size: 20px\"></span></i>&nbsp&nbsp&nbsp&nbsp</span>";
												} else {
													txt += "<span><i class=\"far fa-heart\" id=\"like"
															+ parseInt(i
																	+ floor
																	* times)
															+ "\" style=\"font-size: 30px ; color: white;\"><span style=\"font-size: 20px\"></span></i>&nbsp&nbsp&nbsp&nbsp</span>";
												}
												...
												txt += "type : \"POST\","
												txt += "data : {Articlefloor : $(\"#articlefloor"
														+ parseInt(i + floor
																* times)
														+ "1\").val() , Memberid : $(\"#CustomermessageboardMemberid\").val(),},"
												txt += "success : function(data) {checkpressornot();}})"
												txt += "})<\/script>"

												// 留言button id = floor + i  ex:floor0, floor1
												txt += "<span><button class=\"button\"><i class=\"fas fa-comment-dots\" id=\"floor"
														+ parseInt(i + floor
																* times)
														+ "\" style=\"font-size: 20px\"><span style=\"font-size: 20px\">&nbsp&nbsp留言</span></i></button></span>";
												txt += "<span>&nbsp&nbsp&nbsp&nbsp&nbsp<button class=\"button1\" id=\"report"
														+ parseInt(i + floor
																* times)
														+ "\" ><i class=\"fas fa-exclamation-circle\" style=\"font-size: 20px ; font-color:red \"><span style=\"font-size: 20px\">&nbsp&nbsp檢舉</span></i></button></span></div></div>";
												//留言區域 id = floor + i + 1  ex: floor01 , floor11
												txt += "<div class=\"post-footer\" id=\"floor"
														+ parseInt(i + floor
																* times)
														+ "1\"  style=\"display: none\">";
												txt += "<div class=\"input-group\">";
												// 輸入留言區域給予id = leavecontentarea +i
												txt += "<input class=\"form-control\" placeholder=\"Add a comment\" type=\"text\" id=\"leavecontentarea"
														+ parseInt(i + floor
																* times)
														+ "\">";
												txt += "<span class=\"input-group-addon\"><button id=\"articlefloor"
														+ parseInt(i + floor
																* times)
														+ "\" ><i class=\"fa fa-edit\"></i></button>"
												// 放置ajax抓取該articlefloor最大樓數+1的位置  id= customermessageboardResponseFloor + i 
												txt += "<input type=\"text\" id=\"customermessageboardResponseFloor"
														+ parseInt(i + floor
																* times)
														+ "\" style=\"display: none\">"
												...
												txt += "<form id=\"report"
														+ parseInt(i + floor
																* times)
														+ "2\" name=\"report\" action=\"/main/report\">"
												...
												txt += "<input type=\"text\" id=\"articlefloor"
														+ parseInt(i + floor
																* times)
														+ "1\" style=\"display: none\" value=\""
														+ data[i]["customermessageboardArticleFloor"]
														+ "\"></span></form></div>"
												txt += "<div id=\"floor"
														+ parseInt(i + floor
																* times)
												...
												txt += "$(\"#floor"
														+ parseInt(i + floor
																* times)
														+ "\").click(function(){$(\"#\"+this.id+\"1\").toggle(300);})"
												txt += "<\/script>"
												txt += "<script>"
												//寫入aftercomment傳出留言之後自動搜尋留言寫出																																																																																																																																					
												txt += "function aftercomment"
														+ parseInt(i + floor
																* times)
														+ "(){var txt1 = \"\" ;$(\"#floor"
														+ parseInt(i + floor)
														+ "11\").empty();$.ajax({url : \"/main/searchcomment/\",type : \"POST\",data : { CustomermessageboardArticleFloor : $(\"#articlefloor"
														+ parseInt(i + floor
																* times)
														+ "1\").val() },success : function(data) {for (var k = 0; k < data.length; k++) {txt1 += \"<ul class=\\\"comments-list\\\" >\";txt1 += \"<li class=\\\"comment\\\"><a class=\\\"pull-left\\\" href=\\\"/main/setting/\"+data[k][\"customermessageboardMemberNum\"]+\"\\\"> <img class=\\\"avatar\\\" src=\\\"/admin/memberBeans/getServerPicture/\"+data[k][\"customermessageboardMemberNum\"]+\"\\\" alt=\\\"avatar\\\"></a>\";txt1 += \"<div class=\\\"comment-body\\\"><h4 class=\\\"user\\\">&nbsp;&nbsp;\"+data[k][\"customermessageboardMemberid\"]+\"</h4></div>\";txt1 += \"<p>&nbsp;&nbsp;\"+data[k][\"customermessageboardMessage\"]+\"</p></div></li></ul>\"}$(\"#floor"
														+ parseInt(i + floor
																* times)
														+ "11\").html(txt1);}})}"
												...
												txt += "$(\"#articlefloor"
														+ parseInt(i + floor
																* times)
														+ "\").mousedown(function(){$.ajax({url : \"\/main\/searchthelastofcomment\/\",type : \"POST\",data : { CustomermessageboardArticleFloor : $(\"#articlefloor"
														+ parseInt(i + floor)
														+ "1\").val()},success : function(data) {$(\"#customermessageboardResponseFloor"
														+ parseInt(i + floor)
														+ "\").val(data[0][\"customermessageboardResponseFloor\"] + 1);}})})"
												txt += "<\/script>"
												...
												txt += "$(\"#articlefloor"
														+ parseInt(i + floor
																* times)
														+ "\").mouseup(function() {if($(\"#leavecontentarea"
														+ parseInt(i + floor
																* times)
														+ "\").val() == \"\"){alert(\"不可空白\")}else{$.ajax({url : \"\/main\/public\/\",type : \"POST\",data : {CustomermessageboardMessage : $(\"#leavecontentarea"
														+ parseInt(i + floor
																* times)
														+ "\").val(),CustomermessageboardResponseFloor : $(\"#customermessageboardResponseFloor"
														+ parseInt(i + floor
																* times)
														+ "\").val(),CustomermessageboardArticleFloor : $(\"#articlefloor"
														+ parseInt(i + floor
																* times)
														+ "1\").val(),CustomermessageboardMemberid : $(\"#CustomermessageboardMemberid\").val(),CustomermessageboardStatus : 1,CustomermessageboardMemberNum : $(\"#CustomermessageboardMemberNum\").val(),},error : function() {$(\"#leavecontentarea"
														+ parseInt(i + floor
																* times)
														+ "\").val('');aftercomment"
														+ parseInt(i + floor
												...
												txt += "$(\"#floor"
														+ parseInt(i + floor
																* times)
														+ "\").mousedown(function(){aftercomment"
														+ parseInt(i + floor
																* times)
														+ "()})"
												txt += "<\/script>"
											...
											$("#searchend")
													.val(
															data[datalength]["customermessageboardArticleFloor"]);
										}
						...
						$
								.ajax({
									...
									success : function(data) {
										$("#customermessageboardArticleFloor")
												.val(
														data[0]["customermessageboardArticleFloor"] + 1);
									}
								})
					});
	$("#messageboardsubmit").mouseup(
			function() {
	...
	$(document)
			.ready(
								...
								.ajax({
									...
									success : function(data) {
										...
										for (var i = 0; i < data.length; i++) {
											...
											txt += "CustomermessageboardArticleFloor : $(\"#articlefloor"
													+ i + "1\").val() },"
					...
					txt += "alt=\\\"avatar\\\"></a>\";txt1 += \"<div class=\\\"comment-body\\\">"
											txt += "<h4 class=\\\"user\\\">&nbsp;&nbsp;\"+data[i][\"customermessageboardMemberid\"]+\"</h4></div>\";txt1 += \"<p>&nbsp;&nbsp;\"+data[i][\"customermessageboardMessage\"]+\"</p></div></li></ul>\"}$(\"#floor"
													+ i
													+ "11\").html(txt1)}})}"
											//  取出articlefloor 並放置該article的最大樓數+1
											txt += "$(\"#articlefloor"
						...
						function report() {
							$(".button1").click(
									function() {
										document.getElementById(this.id + "2")
												.submit();
									})
						}
						...
						txt += "alt=\\\"avatar\\\"></a>\";txt1 += \"<div class=\\\"comment-body\\\">"
																	...
																	txt += "$(\"#articlefloor"
																			+ i
																			+ "\").mouseup(function() {$.ajax({url : \"\/main\/public\/\",type : \"POST\",data : {CustomermessageboardMessage : $(\"#leavecontentarea"
																			+ i
																			+ "\").val(),CustomermessageboardResponseFloor : $(\"#customermessageboardResponseFloor"
																			+ i
																			+ "\").val(),CustomermessageboardArticleFloor : $(\"#articlefloor"
