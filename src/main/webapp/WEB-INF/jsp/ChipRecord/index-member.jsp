	...
	<form class="form-signin" name="form1" id="form1" action=""
		...
		<div>
			...
			<div class="form-group row">
				<div class="col-sm-12">
					<input id='chipRecordMemberid' name="chipRecordMemberid" type="text" value="${memberP.chipRecordMemberid}"
						placeholder="ChipRecord-MemberId:" class="form-control">
					<div>${ErrorMsg.highwayName}</div>
				</div>
			</div>
			<div class="form-group row">
				<div class="col-sm-12">
					<input id='chipRecordRecord' name="chipRecordRecord" type="text" value="${memberP.chipRecordRecord}"
						placeholder="ChipRecodr-Record:" class="form-control">
					<div>${ErrorMsg.highwayName}</div>
				</div>
			</div>
		...
		$("#insert1").click(function() {
			...
			$.ajax({
				...
				success : function(ajaxres) {
					...
					$(location).attr("href", "/admin/chiprecord/list");
...
// 		$(location).attr("href","/admin/member/findByIdBetween?id1="+$('#id1').val()+"&id2="+$('#id2').val());
		$.ajax({
			url:"/admin/member/findByIdBetween?id1="+$('#id1').val()+"&id2="+$('#id2').val(),
			type:"GET",
//  			data:{id1:$('#id1'), id2:$('#id2')},
			success:function(data){
				alert(JSON.stringify(data))
				
				$(location).attr("href","/admin/chiprecord/list?id1="+$('#id1').val()+"&id2="+$('#id2').val());
			},
		     error:function(data){
		    	 alert("資料搜尋失敗");
		     }
